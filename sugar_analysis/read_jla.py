...
#    EBV = read_lc_jla(sn_name)['@MWEBV']
#    z = read_lc_jla(sn_name)['@Z_HELIO']
...
def read_lc_jla(sn_name, model = None, sad_path = '../../'):
    ...
    if cov in os.listdir(sad_path+'sugar_analysis_data/data/jla_data/jla_light_curves/'):
        # with open(sad_path+'sugar_analysis_data/data/jla_data/jla_light_curves/' + cov, 'r') as table:
        #     size = table.readline()
        #     cov_file = np.genfromtxt(table)

        infile = open(sad_path+'sugar_analysis_data/data/jla_data/jla_light_curves/' + cov, 'r')
        infile.readline()
        table = [line.encode('utf-8') for line in infile]
        cov_file = np.genfromtxt(table)
        
        data['fluxcov'] = cov_file
        if head['@ZPERR_STANDARD::U'] == 0.1:
            zp_uncer = np.zeros(len(band))
            for i, b in enumerate(band): 
                if b == 'jla_STANDARD::U':
                    zp_uncer[i] = 0.1
            data['fluxcov'] += zp_uncer * zp_uncer[:, np.newaxis]
